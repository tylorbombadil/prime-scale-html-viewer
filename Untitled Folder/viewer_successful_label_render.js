...
// Draw vertical prime ticks with labels for selected primes
if (scale.metadata && scale.metadata.primes && scale.metadata.log_prime_positions) {
  ...
  const tickHeight = 10;
  const labelPrimes = new Set([2, 3, 5, 7, 11, 13, 17, 19, 23]);

  ctx.fillStyle = '#888';

  primes.forEach((prime, i) => {
    const x = positions[i] * canvas.width;
    const y = canvas.height / 2 - tickHeight / 2;
    ctx.fillRect(x, y, 1, tickHeight);

    if (labelPrimes.has(prime)) {
      ctx.fillStyle = '#444';
      ctx.font = '10px monospace';
      ctx.fillText(prime.toString(), x + 2, y - 2);
      ctx.fillStyle = '#888';
    }
  });
}
